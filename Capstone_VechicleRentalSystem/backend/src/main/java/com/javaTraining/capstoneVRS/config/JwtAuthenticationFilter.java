...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null && jwtComponent.validateToken(token) && !jwtComponent.isTokenExpired(token)) {
            ...
            log.debug("Authenticated request path={} email={} role={}", request.getRequestURI(), email, role);
        } else if (token != null) {
            log.warn("Token validation failed for request path={}", request.getRequestURI());
        }
